...
@section('content')
<section class="edit-certificate-page" style="min-height: 100vh; padding-top: 96px; padding-bottom: 80px; background: radial-gradient(circle at top, rgba(59,130,246,0.3) 0, transparent 55%), radial-gradient(circle at bottom, rgba(139,92,246,0.25) 0, transparent 55%);">
    <div class="container" style="max-width: 720px; margin: 0 auto;">
        <div style="margin-bottom: 28px;">
            <p style="font-size: 0.85rem; color: #94a3b8; letter-spacing: 0.08em; text-transform: uppercase; margin-bottom: 6px;">
                Certificates
            </p>
            <h1 style="font-size: 2rem; font-weight: 800; color: #f8fafc; margin-bottom: 6px;">
                Edit certificate
            </h1>
            <p style="font-size: 0.95rem; color: #9ca3af;">
                Update the details of this credential. Changes will be reflected on your public portfolio.
            </p>
        </div>
        ...
        <div style="background: rgba(15,23,42,0.9); border-radius: 20px; padding: 24px 24px 20px; border: 1px solid rgba(148,163,184,0.45); box-shadow: 0 20px 50px rgba(15,23,42,0.9); backdrop-filter: blur(20px);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px;">
                <div>
                    <h2 style="font-size: 1.05rem; font-weight: 600; color: #e5e7eb; margin-bottom: 2px;">
                        {{ $certificate->title }}
                    </h2>
            ...
            <form method="POST" action="{{ route('owner.certificates.update', $certificate) }}" style="display: flex; flex-direction: column; gap: 18px; margin-top: 8px;">
                @csrf
                @method('PUT')
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit,minmax(160px,1fr)); gap: 14px;">
                    <div style="display: flex; flex-direction: column; gap: 6px;">
                        ...
                        <input type="date" name="issued_date" required
                               value="{{ old('issued_date', \Carbon\Carbon::parse($certificate->issued_date)->format('Y-m-d')) }}"
                    ...
                    <div style="display: flex; flex-direction: column; gap: 6px;">
                        ...
                        <input type="date" name="expiry_date"
                               value="{{ old('expiry_date', $certificate->expiry_date ? \Carbon\Carbon::parse($certificate->expiry_date)->format('Y-m-d') : '') }}"
                               style="width: 100%; padding: 9px 12px; border-radius: 10px; border: 1px solid rgba(148,163,184,0.6); background: rgba(15,23,42,0.85); color: #e5e7eb; font-size: 0.9rem; outline: none;">
                    </div>
                </div>
                ...
                <div style="display: flex; flex-direction: column; gap: 6px;">
                    ...
                    <textarea name="description" rows="4"
                              style="width: 100%; padding: 10px 14px; border-radius: 10px; border: 1px solid rgba(148,163,184,0.6); background: rgba(15,23,42,0.85); color: #e5e7eb; font-size: 0.95rem; outline: none; resize: vertical;">{{ old('description', $certificate->description) }}</textarea>
                </div>
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; gap: 12px; margin-top: 18px;">
                    ...
                    <button type="submit"
                            style="flex: 1; padding: 10px 14px; border-radius: 999px; border: none; background: linear-gradient(135deg,#22c55e,#4ade80); color: #022c22; font-weight: 600; font-size: 0.9rem; cursor: pointer; box-shadow: 0 12px 30px rgba(34,197,94,0.45);">
                        Update certificate
                    </button>
                </div>
            </form>
